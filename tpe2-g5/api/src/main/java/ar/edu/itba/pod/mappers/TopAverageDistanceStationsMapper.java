...
public class TopAverageDistanceStationsMapper implements Mapper<Integer, Trip, Integer, Pair<Integer, Double>> {
    ...
    public void map(Integer key, Trip trip, Context<Integer, Pair<Integer, Double>> context) {
        if (stationMap.containsKey(trip.getStartStation()) && stationMap.containsKey(trip.getEndStation())) {
            if (trip.getEndStation() != trip.getStartStation() && trip.getIsMember() == 1) {
                ...
                context.emit(trip.getStartStation(), new Pair<>(1, startStation.haversineDistance(endStation)));
            }
        }
    }
}
